...
def main():
    # Config
    scrape_interval_seconds = 5       # how often to ping/update metrics
    metrics_port = 8000               # exporter port (Prometheus will scrape this)

    destinations = load_domains("domain.yaml")
    if not destinations:
        raise SystemExit("No domains found in domain.yaml (expected key: domains)")

    print(f"Starting exporter on :{metrics_port}, pinging: {destinations}")
